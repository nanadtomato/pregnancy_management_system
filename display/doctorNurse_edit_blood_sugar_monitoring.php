...
// Fetch existing blood sugar monitoring record
$stmt = $conn->prepare("SELECT * FROM blood_sugar_monitoring WHERE id = ? AND patient_id = ?");
...
if (!$record) {
    die("Blood sugar monitoring record not found.");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update the blood sugar monitoring record in the database
    $stmt = $conn->prepare("UPDATE blood_sugar_monitoring SET 
        monitoring_datetime = ?, pre_post_breakfast = ?, pre_post_lunch = ?, 
        pre_post_dinner = ?, pre_bed = ?, notes = ? 
        WHERE id = ? AND patient_id = ?");
    
    $stmt->bind_param("sssddssi", $monitoring_datetime, $pre_post_breakfast, $pre_post_lunch, 
        $pre_post_dinner, $pre_bed, $notes, $record_id, $patient_id);

    if ($stmt->execute()) {
        $success = "Blood sugar monitoring record updated successfully.";
    } else {
...
<head>
    ...
    <title>Edit Blood Sugar Monitoring</title>
...
<main>
    <h2 class="mb-4 text-center">Edit Blood Sugar Monitoring</h2>
        ...
        <form method="POST">
            <div class="row g-3">
                ...
                <div class="col-md-12">
                    ...
                    <input type="number" step="0.01" name="pre_post_breakfast" class="form-control" value="<?= htmlspecialchars($record['pre_post_breakfast']) ?>" required>
                </div>
                <div class="col-md-12">
                    <label for="pre_post_lunch" class="form-label">Pre/Post Lunch (mmol/L)</label>
                    <input type="number" step="0.01" name="pre_post_lunch" class="form-control" value="<?= htmlspecialchars($record['pre_post_lunch']) ?>" required>
                </div>
                <div class="col-md-12">
                    <label for="pre_post_dinner" class="form-label">Pre/Post Dinner (mmol/L)</label>
                    <input type="number" step="0.01" name="pre_post_dinner" class="form-control" value="<?= htmlspecialchars($record['pre_post_dinner']) ?>" required>
                </div>
                <div class="col-md-12">
                    <label for="pre_bed" class="form-label">Pre Bed (mmol/L)</label>
                    <input type="number" step="0.01" name="pre_bed" class="form-control" value="<?= htmlspecialchars($record['pre_bed']) ?>" required>
                </div>
            </div>
            ...
            <div class="row g-3">
                <div class="col-md-12">
                    ...
                    <textarea name="notes" class="form-control" rows="4"><?= htmlspecialchars($record['notes']) ?></textarea>
                </div>
            </div>
            ...
            <div class="mt-4 text-center">
                <button type="submit" class="btn btn-pink">Update Blood Sugar Monitoring</button>
