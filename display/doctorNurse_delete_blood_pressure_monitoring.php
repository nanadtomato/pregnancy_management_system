...
// Delete the blood pressure monitoring record
$stmt = $conn->prepare("DELETE FROM blood_pressure_monitoring WHERE id = ? AND patient_id = ?");
$stmt->bind_param("ii", $record_id, $patient_id);

if ($stmt->execute()) {
    // Redirect back to the blood pressure monitoring view page
    header("Location: doctorNurse_view_blood_pressure_monitoring.php?user_id=$patient_user_id");
    exit();
} else {
